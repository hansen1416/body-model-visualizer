...
# this is the 4dhumans visualizer
...
class Visualizer(nn.Module):
    ...
    def __init__(self, cfg, hmar):
        ...
        self.render = Renderer(
            focal_length=self.cfg.EXTRA.FOCAL_LENGTH, img_res=256, faces=self.faces_cpu
        )
    ...
    def render_single_frame(
        ...
        smpl_output = self.hmar.smpl(
            **{k: v.float().cuda() for k, v in pred_smpl_params.items()}, pose2rot=False
        )
    ...
    def render_video(self, final_visuals_dic):
        ...
        frame_size = (
            self.cfg.render.output_resolution * NUM_PANELS,
        ...
        if len(tracked_ids) > 0:
            ...
            if "HUMAN" in self.cfg.render.type:
                ...
                ids_x = np.logical_and(
                    ...
                    == int(self.cfg.render.type.split("TRACKID_")[1].split("_")[0]),
                )
        ...
        if "label" in final_visuals_dic.keys():
            ...
            rendered_image_final = self.visualize_labels_arrow(
                rendered_image_final,
                tracked_labels,
                tracked_bbox[ids_x] * ratio_,
                tracked_colors,
                tracked_ids_x,
                tracked_smpl[ids_x],
                tracked_cameras[ids_x],
