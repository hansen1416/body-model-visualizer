...
def get_part_idxs():
    body_idxs = np.asarray(
        [idx for idx, val in enumerate(SMPLX_PARTS.values()) if "body" in val]
    )

    hand_idxs = np.asarray(
        [idx for idx, val in enumerate(SMPLX_PARTS.values()) if "hand" in val]
    )
    ...
    face_idxs = np.asarray(
        [idx for idx, val in enumerate(SMPLX_PARTS.values()) if "face" in val]
    )
    head_idxs = np.asarray(
        [idx for idx, val in enumerate(SMPLX_PARTS.values()) if "head" in val]
    )
    flame_idxs = np.asarray(
        [idx for idx, val in enumerate(SMPLX_PARTS.values()) if "flame" in val]
    )
    foot_idxs = np.asarray(
        [idx for idx, val in enumerate(SMPLX_PARTS.values()) if "foot" in val]
    )
